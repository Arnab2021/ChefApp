...
export default class ChatScreen extends Component {
  ...
  async componentDidMount() {
    ...
    const { item } = this.props.route.params
    const sender_id = await getData('chef_id')
    const sender_name = item.chef_name
    const sender_image = item.ChefPic

    const receiver_id = item.user_id
    const receiver_name = item.user_name
    const receiver_image = item.UserPic
  ...
  async onSend(messages) {
    ...
    try {
      ...
      const data = {
        chef_id: sender_id,
        chef_name: sender_name,
        ChefPic: sender_image,
        user_id: receiver_id,
        user_name: receiver_name,
        UserPic: receiver_image,
