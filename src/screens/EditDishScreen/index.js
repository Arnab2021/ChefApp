...
export default class EditDishScreen extends Component {
  constructor(props) {
    super(props);
    this.state = {
      loader: false,
  ...
  async _gatherDishData() {

    const { cuisineItem, DeliveryTypeItem, dishType, dietaryItem, orderTypes } = this.state
    const dish_array = this.props.route.params.dish_array

    const dish_name = dish_array.dish_name
    const description = dish_array.dish_name
    const price = dish_array.price
    const delivery_fees = dish_array.delivery_fees
    const selected_delivery_type = DeliveryTypeItem.indexOf(dish_array.delivery_type)
    const selected_cuisine = cuisineItem.indexOf(dish_array.cuisine)
    const selected_dishtype = dishType.indexOf(dish_array.dish_type)
    const ingredients = dish_array.ingredients
    const selected_dietary_pref = dietaryItem.indexOf(dish_array.dietary_pref)
    const quantity = dish_array.quantity
    const selected_ordertype = orderTypes.indexOf(dish_array.order_type)
    const picture_url = dish_array.pic_url
    ...
    this.setState({
      ...
      selectedCuisine: selected_cuisine,
      selectedDishType: selected_dishtype,
      ingredients: ingredients,
      selectedDietary: selected_dietary_pref,
      quantity: quantity,
      selectedOrderType: selected_ordertype,
      picture_url: picture_url,
      dish_id: dish_array.dish_id
    })
  ...
  async _editDish() {
    ...
    const response = await callApi('updateDish', "POST", param)
    ...
    return (
      <KeyboardAwareScrollView contentContainerStyle={{ flexGrow: 1 }} >
        ...
        <View style={styles.main}>
          ...
          <TouchableOpacity style={styles.submitButton} onPress={() => this._editDish()} disabled={(this.state.loader)?true: false}>
            {
              (this.state.loader) ?
